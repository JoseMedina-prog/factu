...
@section('content')
<x-page-header title="Nueva Nota de Crédito" subtitle="Crear nota de crédito para devolución" :back="route('credit-notes.index')" />
        ...
        <div class="lg:col-span-2 space-y-6">
            <x-card>
                ...
                <div class="p-6">
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            <label class="label">Factura Relacionada</label>
                            <select name="invoice_id" class="input">
                                <option value="">Ninguna</option>
                                @foreach($invoices as $invoice)
                                    <option value="{{ $invoice->id }}">{{ $invoice->number }} - {{ $invoice->client->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="notes" rows="2" class="input" placeholder="Motivo de la nota crédito">{{ old('notes') }}</textarea>
                        </div>
                    </div>
                </div>
            </x-card>
            ...
            <x-card>
                <div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
                    <div>
                        <h2 class="text-lg font-semibold text-slate-900">Ítems de la nota</h2>
                ...
                <div class="p-6">
                    <div id="itemsContainer" class="space-y-3">
                        ...
                        @foreach($oldItems as $index => $oldItem)
                            @include('credit_note._item_row', ['index' => $index, 'item' => $oldItem, 'products' => $products])
                        @endforeach
                    </div>
                </div>
            </x-card>
        ...
        <div class="space-y-6">
            ...
            <x-card>
                ...
                <div class="p-6 space-y-3">
                    <x-button :href="route('credit-notes.index')" variant="outline" :block="true" icon="x">Cancelar</x-button>
                    <x-button type="submit" variant="primary" :block="true" icon="check">Crear Nota de Crédito</x-button>
                </div>
            </x-card>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    addBtn.addEventListener('click', function () {
        ...
        refreshListeners(container.lastElementChild);
        itemIndex++;
    });
            ...
            <div class="flex-1 space-y-3">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-12 gap-3">
                    ...
                    <div class="col-span-1 sm:col-span-2">
                        ...
                        <input type="number" name="items[__INDEX__][tax_rate]" step="0.01" min="0" max="100" value="0" class="input tax">
                    </div>
